...
- ` Controller Mode` - Choose what PS4 mode the device will use, either `Controller` or `Arcade Stick`.
  - `Controller` - The device will function as a native PS4 controller
  - `Arcade Stick` - The device will function as a licensed PS4 arcade stick, provided that an authentication device is connected and the PS Passthrough add-on is enabled and properly configured.
