...
- **PS5 主机的 PS5 游戏**：使用 [`PS4 输入模式`](../web-configurator/menu-pages/01-settings.mdx#additional-ps4-settings)。
- **Xbox One 和 Xbox Series X|S 游戏**：使用 [`Xbox One 输入模式`](../web-configurator/menu-pages/01-settings.mdx#additional-xbox-one-settings)。
...
1. 如果能够找到用户友好且完全合法的实现方法，[PS4 输入模式](../web-configurator/menu-pages/01-settings.mdx#additional-ps4-settings) 的实现可能会被进一步扩展支持。

2. **当前支持方式：**
   - **PS5**：仅通过 passthrough 认证支持，具体见 [PS5 输入模式](../web-configurator/menu-pages/01-settings.mdx#additional-ps5-settings)。
   - **Xbox One 和 Xbox Series 主机**：仅通过 passthrough 认证支持，具体见 [Xbox One 输入模式](../web-configurator/menu-pages/01-settings.mdx#additional-xbox-one-settings)。
...
- **只运行一个控制器的嵌入式 Web 配置工具**。不能同时运行多个。
...
### **诸如 RGB LED、玩家 LED 和 OLED 显示屏等附加功能是否会影响性能？**

不会！RP2040 处理器拥有两个核心：

- GP2040-CE **专门利用一个核心处理玩家输入的读取、处理和发送任务**。
- 所有附加功能（例如 LED 和显示屏）都运行在第二核心上。
...
GP2040-CE 的文档和 Web 配置工具都提供了一个下拉菜单，您可以将按钮标签更改为更熟悉的控制器布局。有关按键映射的详细信息，请参阅 [GP2040-CE 使用文档](../usage.mdx#buttons)。
...
- `lwIP` 库实现了一个 HTTP 服务器，用于服务内嵌的 React 应用程序和 Web 配置 API。
