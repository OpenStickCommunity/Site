...
并非所有的 Android 和 iOS 设备都支持 Web 配置。在我们的测试中，一些 Android 设备可以工作，而另一些则不行。大多数基于 USB-C 的 iOS 设备都能工作。我们没有记录哪些设备可以或不可以工作。

:::

## 在 Windows PC 上访问 Web 配置时遇到问题

如果您的 Windows PC 无法访问 GP2040-CE 控制器的内置 Web 配置，请尝试以下步骤：
...
3 - ** 检查 URL**
  ...
  - Web 配置的 URL 是 [http://192.168.7.1](http://192.168.7.1)。
...
Linux 发行版可能需要一些额外的步骤才能访问 Web 配置器；请参阅 [Linux 安装](https://www.google.com/search?q=%23linux-setup)。
...
Web 配置器会自动运行，您只需要能够访问它即可。某些配置会自动设置路由，因此现在请在浏览器中尝试 [http://192.168.7.1](http://192.168.7.1)。如果无法加载，请尝试通过以下命令手动为接口配置 IP：`sudo ifconfig enp6s0f1u3 192.168.7.2`。
