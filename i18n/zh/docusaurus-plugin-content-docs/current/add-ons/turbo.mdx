...
通过按住某个输入并同时按下 Turbo 按钮，可以启用/禁用 Turbo 模式。这会导致该输入以快速连续的方式重复发送（根据 网页配置器中的 `Turbo Shot Count` 设置）。
...
对于 Turbo LED，将 LED 的一侧连接到 网页配置器中分配的 GPIO 引脚。将 LED 的另一侧连接到 GND。
