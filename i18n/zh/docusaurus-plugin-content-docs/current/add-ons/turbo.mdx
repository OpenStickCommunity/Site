...
用途：此附加功能用于更改控制行为，使得按住一个按钮时能够快速触发一系列单独的按钮按下操作。

通过按住某个输入并同时按下 Turbo 按钮，可以启用/禁用 Turbo 模式。这会导致该输入以快速连续的方式重复发送（根据 Web 配置器中的 `Turbo Shot Count` 设置）。
...
![GP2040-CE 配置器 - Turbo 附加功能](../assets/images/gpc-add-ons-turbo.png)
...
对于 Turbo LED，将 LED 的一侧连接到 Web 配置器中分配的 GPIO 引脚。将 LED 的另一侧连接到 GND。
