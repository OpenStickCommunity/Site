...
用途：倾斜输入附加功能允许用户通过左侧和右侧模拟摇杆发送一定百分比的最大方向输入（例如，在右侧模拟摇杆上发送 65% 的向下-右输入）。

![GP2040-CE 配置器 - 倾斜输入附加功能](../assets/images/gpc-add-ons-tilt.png)
...
- `Tilt SOCD Mode` - 选择默认 SOCD 清理模式（中立、最后胜出、先胜出）。
...
此附加功能需要额外的按钮或开关，因为它完全禁止将主方向键 (DPad) 设置为左侧或右侧模拟摇杆。

### 安装

对于每个按钮，将按钮的一侧连接到 Web 配置器中分配的对应 GPIO 引脚。将按钮的另一侧连接到 GND。

## 其他备注

- 由于此附加功能禁止将方向键 (DPad) 设置为左侧或右侧模拟摇杆，使用快捷键 `DPad Left Analog` 和 `DPad Right Analog` 会禁用方向键，而使用 `DPad Digital` 会重新启用方向键。
- 并非所有倾斜输入的模拟引脚都需要设置，但未设置的引脚将无法使用该输入，除非通过 Web 配置器重新映射输入。
